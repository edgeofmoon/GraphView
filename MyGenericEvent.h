...
class MyModifierState{
public:
	MyModifierState(int modifiers = 0);
...
protected:
	int mModifiers;
	//bool mShiftActive = 0001b;
	//bool mCtrlActive = 0010b;
	//bool mAltActive = 0100b;
};
...
public:
	...
	// opengl interface
	static MyGenericEvent GenerateKeyboardKeyEvent(unsigned char c, MyKeyState state, int x, int y, MyModifierState modifier = MyModifierState());
	static MyGenericEvent GenerateMouseKeyEvent(MyMouseKey button, MyKeyState state, int x, int y, MyModifierState modifier = MyModifierState());
	static MyGenericEvent GenerateMouseMoveEvent(MyMouseKey button, MyKeyState state, int x, int y, MyModifierState modifier = MyModifierState());
	...
	virtual MyModifierState GetModiferState() const {
		return mModifers;
	}
...
protected:
	...
	MyGenericEvent(unsigned char c, MyKeyState state, int x, int y, MyEventSource source, MyModifierState modifier, int eveIdx);
	// mouse press or release or move
	MyGenericEvent(MyMouseKey button, MyKeyState state, int x, int y, MyEventSource source, MyModifierState modifier, int eveIdx);
	// resize event
